...
commands.push(['bind', 'Notify this channel when events happen', async (bot, userName, userId, channelId) => {
...
commands.push(['sync', 'Update user roles in all bound servers, based on google sheet', async (bot, userName, userId, channelId) => {
    bot.sendMessage({
        to: channelId,
...
commands.push(['auth', 'Authenticate as an administrator', async (bot, userName, userId, channelId, message, evt, args) => {
    if (args.length <= 1) {
...
commands.push(['set-password', 'Change the admin password, un-authenticating everyone', async (bot, userName, userId, channelId, message, evt, args) => {
    if (args.length <= 1) {
        bot.sendMessage({
            to: channelId,
            message: 'Missing argument',
...
commands.push(['set-url', 'Change which google sheet I should read from when synchronizing', async (bot, userName, userId, channelId, message, evt, args) => {
    if (args.length <= 1) {
        bot.sendMessage({
            to: channelId,
            message: 'Missing argument',
...
commands.push(['set-ranks-column', 'Change which google sheet column I should read ranks from when synchronizing', async (bot, userName, userId, channelId, message, evt, args) => {
    if (args.length <= 1) {
        bot.sendMessage({
            to: channelId,
            message: 'Missing argument',
...
commands.push(['set-names-column', 'Change which google sheet column I should read names from when synchronizing', async (bot, userName, userId, channelId, message, evt, args) => {
    if (args.length <= 1) {
        bot.sendMessage({
            to: channelId,
            message: 'Missing argument',
